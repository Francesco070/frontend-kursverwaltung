  ...
  <v-dialog v-model="dialog" max-width="600" persistent transition="dialog-top-transition">
    <v-card class="py-4">
      ...
      <v-card-actions class="px-6">
        ...
        <v-btn @click="addDozent()" color="primary">Hinzufügen</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
const props = defineProps<{ addDozentDialog: boolean }>();
const emit = defineEmits(['update:addDozentDialog']);

const dialog = computed({
  get: () => props.addDozentDialog,
  set: (value) => emit('update:addDozentDialog', value),
});
...
async function addDozent() {
  let errorInAddDozenten = false;

  const {execute, error, data} = useFetch(url, {immediate: false}).post(dozent.value).json();

  await execute();

  if (error.value) {
    addSnackBar('error', `Fehler beim hinzufügen von Dozent.`);
    errorInAddDozenten = true;
